  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isProject && icons.length > 0 && (
          <div className="flex flex-wrap justify-center gap-6 w-full mt-2">
            {icons.map(({ Icon, label, color }) => (
              ...
              >
                <Icon size={48} />
                <Typography variant="body2" className="mt-2" sx={{ color: "#f8f1da" }}>
                  {label}
                </Typography>
              </div>
            ))}
        ...
        {/* For Skill Images */}
        {!isProject && images.length > 0 && (
          <div className="flex flex-wrap justify-center gap-6 w-full mt-2">
            {images.map(({ src, label, hoverColor }) => (
              <div
                key={label}
                title={label}
                aria-label={label}
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.filter = `grayscale(0%) drop-shadow(0 0 5px ${hoverColor})`;
                  e.currentTarget.style.transform = "translateY(-8px)";
                }}
                onMouseLeave={(e) => {
